...
parse_date_strptime_valid_test(void)
{
	...
	for (size_t index = 0; index < lengthof(format_tests); index++) {
		...
		isnt(ptr, NULL, "parse string '%s' using '%s' (result '%s')",
		     text, fmt, TOSTR(tnt_strftime, 0, "%FT%T%z", &date));
	}
...
snprint_test(void)
{
	...
	char *res;
	res = TOSTR(datetime_to_string, 0, &d);
	is(strcmp(res, zero_date), 0, res);

	res = TOSTR(datetime_strftime, 0, zero_fmt, &d);
	is(strcmp(res, zero_date), 0, res);

	res = TOSTR(tnt_strftime, 0, zero_fmt, &t);
	is(strcmp(res, zero_date), 0, res);
	...
	res = TOSTR(interval_to_string, 0, &i);
