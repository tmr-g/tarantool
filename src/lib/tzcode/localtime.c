...
int
tnt_tm_snprint(char *buf, int len, const struct tnt_tm *x)
{
	int total = 0;

	if (x == NULL) {
		SNPRINT(total, snprintf, buf, len, "tnt_tm(null)");
		return total;
	}

	SNPRINT(total, snprintf, buf, len, "tnt_tm(x:%p tm_epoch:%ld tm_nsec:%d tm_gmtoff:%ld tm_tzindex:%hd "
